...
describe("Order repository test", () => {
  ...
  it("should update a order with a new order item", async () => {
    const customerRepository = new CustomerRepository();
    const customer = new Customer("123", "Customer 1");
    const address = new Address("Street 1", 1, "Zipcode 1", "City 1");
    customer.changeAddress(address);
  ...
  it("should find a Order", async () => {
    ...
    const orderFromOrderRepository = await orderRepository.find("123");
    expect(order).toMatchObject(orderFromOrderRepository)
  });
  ...
  it("should find all orders", async () => {
    ...
    const order2 = new Order("2", "1234", [orderItem2]);
    ...
    orders.forEach(order => {
      if (order.id === "1") {
        expect(order).toMatchObject(order1);
      } else {
        expect(order).toMatchObject(order2);
      }
    })
  });
